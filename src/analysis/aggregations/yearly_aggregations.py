...
data_service = DataService(client="yearly_metrics", augment=False)

client = MongoInstance("traffic_analysis")
data = client.select_collection("vehicle")

def analyze_yearly(collection):
    """
    CHANGE
    ...
    pipeline = [
        {
            "$addFields": {
                "year": {
                    "$toInt": { "$arrayElemAt": [{ "$split": ["$dmy", "_"] }, 2] }
                }
            }
        },
        {
    ...
    for doc in results:
        try:
            data_service.store_yearly_data(doc)
...
def main_year():
    ...
    # Perform analysis
    analysis_results = analyze_yearly(client)
